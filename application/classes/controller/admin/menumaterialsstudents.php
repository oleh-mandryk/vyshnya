...
/*
 * Меню для методичних матеріалів студентам стаціонару
 */
class Controller_Admin_Menumaterialsstudents extends Controller_Admin {
    ...
    public function before() {
        ...
        $this->template->page_title = 'Меню для методичних матеріалів студентам стаціонару';
        $this->template->title = 'Меню для методичних матеріалів студентам стаціонару';
    }
    
    public function action_index() {
    
        $menu_materials_students = ORM::factory('menumaterialsstudents')->fulltree();
        
        $info_ok = $this->session->get('message_admin');
        
        $content = View::factory('admin/menu_materials_students/menu_materials_students_index_view', array(
    ...
    public function action_add_main() {
        ...
        if (isset($_POST['submit'])) {
            ...
            $menu_materials_students = ORM::factory('menumaterialsstudents');
            ...
            if (($jjj!= 0)and($kkk == 1)) {
                ...
                $this->request->redirect('admin/menumaterialsstudents');
            }
        }
        
        $content = View::factory('admin/menu_materials_students/menu_materials_students_add_main_view')
                ->bind('errors', $errors)
    ...
    public function action_add_subject() {
        ...
        $menu_materials_students_all = ORM::factory('menumaterialsstudents')
            ->where('lvl','=', 1)
            ->order_by('title','ASC')
            ->find_all()
            ->as_array();
        ...
        $content = View::factory('admin/menu_materials_students/menu_materials_students_add_subject_view')
            ->bind('errors', $errors)
            ->bind('data', $data)
    ...
    public function action_add_material() {
        ...
        $content = View::factory('admin/menu_materials_students/menu_materials_students_add_material_view')
            ->bind('errors', $errors)
            ->bind('errors_cap', $errors_cap)
            ->bind('data', $data)
            ->bind('menu_materials_students_ad', $menu_materials_students_ad)
    ...
    public function action_edit() {
        ...
        $menu_materials_students = ORM::factory('menumaterialsstudents', $id);
        ...
        $men = ORM::factory('menumaterialsstudents')->where('id','=',$id)->find();
        ...
        switch($menu_materials_students->lvl) {
            ...
            case 1:
                $link_edit = 'admin/menu_materials_students/menu_materials_students_edit_main_view';
            break;
            
            case 2:
                $link_edit = 'admin/menu_materials_students/menu_materials_students_edit_subject_view';
            break;
            
            case 3:
                $link_edit = 'admin/menu_materials_students/menu_materials_students_edit_material_view';
            break;
        }
        ...
        switch ($menu_materials_students->level()) {
            case '1':
                ...
                if ($menu_materials_students->has_children()) {
                    ...
                    $del_potom = ORM::factory('menumaterialsstudents')->where('scope','=',$scope_main)->where('lvl','=',3)->find_all();
            ...
            case '2':
                $del_potom = ORM::factory('menumaterialsstudents')->where('parent_id','=',$id)->find_all();
                foreach ($del_potom as $del_p) {
                    if ($del_p->id != null) {
                        //Видалення зображення з папки
                        unlink('./media/img/small_img_material/'.$del_p->small_img_url);
